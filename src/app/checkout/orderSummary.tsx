...
export default function OrderSummary() {
  ...
  const [deliveryMethod, setDeliveryMethod] = useState({ method: 'standard', price: 19.99 });
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      ...
      if (savedDeliveryMethod) {
        ...
        // Set default shipping method if none is saved
        const defaultMethod = { method: 'standard', price: 19.99 };
        localStorage.setItem('deliveryMethod', JSON.stringify(defaultMethod));
  ...
  return (
    <Card>
      <CardContent>
        <h2 className="text-2xl font-semibold text-[#154633] mb-3">Order Summary</h2>
        <Separator />
        <div className="space-y-4 mt-10">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-lg">Combat Door Gym Original</h3>
              <p className="text-base">Berry Blast - Qty: {cartData.quantity}</p>
            </div>
            <p className="font-semibold text-[#154633] mt-1">${cartData.total}</p>
          </div>
          <Separator />
          ...
          <div className="flex justify-between font-bold text-lg">
            <p>Total</p>
            <p className="text-[#154633]">${totalWithDelivery}</p>
          </div>
